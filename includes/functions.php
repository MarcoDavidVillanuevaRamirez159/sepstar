...
// Incluir configuración solo si no está incluida
if (!defined('BASE_URL') || !defined('SITE_NAME') || !defined('SITE_URL')) {
...
function get_current_language() {
    ...
    if ($lang != '' && array_key_exists($lang, $available_languages)) {
        ...
        $_SESSION['lang'] = defined('DEFAULT_LANG') ? DEFAULT_LANG : 'es';
    }
...
function load_language_file($section) {
    ...
    try {
        ...
        $default_file = __DIR__ . '/../languages/' . (defined('DEFAULT_LANG') ? DEFAULT_LANG : 'es') . '/' . $section . '.php';
...
function lang_url($url, $lang) {
    ...
    // Eliminar cualquier parámetro lang anterior
    unset($params['lang']);
    // Agregar el nuevo parámetro lang
    $params['lang'] = $lang;
...
function send_email($to, $subject, $message, $headers = '') {
    ...
    $default_headers .= "From: " . (defined('SITE_NAME') ? SITE_NAME : 'Sepstar') . " <" . (defined('ADMIN_EMAIL') ? ADMIN_EMAIL : 'info@localhost') . ">\r\n";
